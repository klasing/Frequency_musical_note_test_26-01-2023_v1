...
public:
	//************************************************************************
	//*                 get_MMDeviceEnumerator_classid
	//************************************************************************
	static const CLSID& get_MMDeviceEnumerator_classid()
	{
		static const CLSID MMDeviceEnumerator_class_id =
			__uuidof(MMDeviceEnumerator);
		return MMDeviceEnumerator_class_id;
	}

	//************************************************************************
	//*                 get_IMMDeviceEnumerator_interface_id
	//************************************************************************
	static const IID& get_IMMDeviceEnumerator_interface_id()
	{
		static const IID IMMDeviceEnumerator_interface_id =
			__uuidof(IMMDeviceEnumerator);
		return IMMDeviceEnumerator_interface_id;
	}
	//************************************************************************
	//*                 get_IAudioClient_interface_id
	//************************************************************************
	...
	//************************************************************************
	//*                 get_IAudioRenderClient_interface_id
	//************************************************************************
	...
	//************************************************************************
	//*                 get_IAudioCaptureClient_interface_id
	//************************************************************************
	...
	public:
		...
		~auto_release() { if (_value != nullptr) _value->Release(); }
	private:
		T*& _value;
	};
	...
	{
		int required_characters = WideCharToMultiByte(CP_UTF8
			, 0
			, wide_string
			, -1
			, nullptr
...
public:
	...
	//*                    <<constructor>>
	//************************************************************************
	audio_device(audio_device&& other) : _device(other._device)
	...
	//************************************************************************
	//*                    supports_sample_type
	//************************************************************************
	template <typename _SampleType>
	constexpr bool supports_sample_type() const noexcept
	{
		return
			std::is_same_v<_SampleType, float>
			|| std::is_same_v<_SampleType, int32_t>
			|| std::is_same_v<_SampleType, int16_t>;
	}
...
private:
	...
	//************************************************************************
	//*                    <<constructor>> audio_device
	//************************************************************************
	audio_device(IMMDevice* device, bool is_render_device) : _device(device)
		, _is_render_device(is_render_device)
	{
		// TODO: Handle errors better.  Maybe by throwing exceptions?
		if (_device == nullptr)
		{
			throw audio_device_exception("IMMDeveice is null.");
		}
	...
	{
		HRESULT hr = _device->Activate(__wasapi_util::get_IAudioClient_interface_id()
			, CLSCTX_ALL
			, nullptr
	...
	void _init_mix_format()
	{
		...
		CoTaskMemFree(device_mix_format_ex);
	}
	...
	{
		if (_running)
			throw audio_device_exception("Cannot connect to running audio device.");

		_user_callback = std::move(callback);
	}

	//************************************************************************
	//*                    _mix_format_matches_type
	//************************************************************************
	template<typename _SampleType>
...
private:
	...
	static std::optional<audio_device> get_default_device(bool output_device)
	{
		...
		HRESULT hr = CoCreateInstance(
			__wasapi_util::get_MMDeviceEnumerator_classid()
			, nullptr
			, CLSCTX_ALL
			, __wasapi_util::get_IMMDeviceEnumerator_interface_id()
		...
		hr = enumerator->GetDefaultAudioEndpoint(output_device ?
			eRender : eCapture, eConsole, &device);
		if (FAILED(hr)) 
			return std::nullopt;
	...
	static std::vector<IMMDevice*> get_devices(bool output_devices)
	{
		...
		hr = enumerator->EnumAudioEndpoints(selected_data_flow
			, DEVICE_STATE_ACTIVE
			, &device_collection
		);
		if (FAILED(hr))
...
private:
	...
	__audio_device_monitor()
	{
		...
		if (FAILED(hr))
			throw audio_device_exception("Could not create enumerator");
	}
	...
	public:
		...
		//*                   <<constructor>>
		//********************************************************************
		WASAPINotificationClient(IMMDeviceEnumerator* enumerator
		...
		//********************************************************************
		HRESULT STDMETHODCALLTYPE OnDeviceStateChanged([[maybe_unused]] LPCWSTR device_id
			, [[maybe_unused]] DWORD new_state
		)
		{
			if (_event != audio_device_list_event::device_list_changed)
				return S_OK;
		...
		//********************************************************************
		HRESULT STDMETHODCALLTYPE OnPropertyValueChanged([[maybe_unused]] LPCWSTR device_id
...
//****************************************************************************
//*                     set_audio_list_callback
//****************************************************************************
template <typename F, typename /* = enable_if_t<is_invocable_v<F>> */>
void set_audio_list_callback(audio_device_list_event event, F&& callback)
{
